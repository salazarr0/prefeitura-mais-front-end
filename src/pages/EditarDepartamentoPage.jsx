...
export default function EditarDepartamentoPage() {
        ...
        const carregarDados = async () => {
            try {
                ...
                const depRes = await axios.get(
                    `https://prefeitura-mais-api-production.up.railway.app/departamentos/${id}`, {
                    headers: { 'Authorization': `Bearer ${token}` }
                });
                
                const dept = depRes.data;
                setNome(dept.nome || "");
                setEndereco(dept.endereco || "");
                setGerente_id(dept.gerente_id || "");

                if (dept.horario_funcionamento) {
                    const partes = dept.horario_funcionamento.split(" ");
                    if (partes.length >= 4) {
                        setDiaInicio(partes[0]);
                        setDiaTermino(partes[2]);
                        
                        const horas = partes[3].split("-");
                        if (horas.length === 2) {
                            setHorario_funcionamentoInicio(horas[0].replace("h", ""));
                            setHorario_funcionamentoTermino(horas[1].replace("h", ""));
                        }
                    }
                }
            } catch (error) {
                console.log(error);
                setMenssagem("Erro ao carregar dados do departamento.");
            } finally {
                setLoading(false);
            }
        };
    ...
    const validarFormulario = () => {
        ...
        if (horario_funcionamentoInicio && horario_funcionamentoTermino) {
            if (horario_funcionamentoInicio > horario_funcionamentoTermino) {
                erros.horario_funcionamento = "O horário de início deve ser menor que o término.";
            }
        }
    ...
    const handleEditarDepartamento = async (e) => {
        e.preventDefault();
        setMenssagem("");
        ...
        try {
            ...
            const dadosDoDepartamento = {
                ...
                endereco: endereco,
                gerente_id: Number(gerente_id)
            };
            
            await axios.put(`https://prefeitura-mais-api-production.up.railway.app/departamentos/${id}`, dadosDoDepartamento, {
                headers: { 'Authorization': `Bearer ${token}` }
            });
    ...
    if (loading) {
        return <div className="min-h-screen flex items-center justify-center bg-gray-50 text-blue-600 font-semibold">Carregando...</div>;
    }
        ...
        <div className="min-h-screen bg-gray-50 flex items-center justify-center px-4 py-8">
            <div className="w-full max-w-2xl bg-white p-8 rounded-2xl shadow-sm border border-gray-100 relative">
                ...
                <div className="flex flex-col gap-4">
                                ...
                                <div className="space-y-3">
                                    ...
                                    <div className="flex items-center gap-2 bg-gray-50 p-2 border border-gray-100 rounded-xl">
                                        <input
                                            ...
                                            min="08:00"
                                            max="17:00"
                                            value={horario_funcionamentoInicio}
                                            className="flex-1 min-w-0 px-3 py-2 border-none bg-white rounded-lg shadow-sm text-sm font-medium text-gray-700 outline-none focus:ring-2 focus:ring-blue-100 cursor-pointer"
                                        ...
                                        <input
                                            ...
                                            min="08:00"
                                            max="18:00"
                                            value={horario_funcionamentoTermino}
                                            className="flex-1 min-w-0 px-3 py-2 border-none bg-white rounded-lg shadow-sm text-sm font-medium text-gray-700 outline-none focus:ring-2 focus:ring-blue-100 cursor-pointer"
                                    ...
                                    <div className="relative">
                                        ...
                                        >
                                            ...
                                            {usuarios.map((usuario) => {
                                                const isVinculadoOutro = departamentos.some(dep => dep.gerente_id === usuario.id && dep.id !== Number(id));
                                                if (usuario.papel === 'funcionario' && !isVinculadoOutro) {
                    ...
                    <button
                        onClick={handleEditarDepartamento}
